...
# NOTE: This version of tunetables supports arbitrary dataset size. 
# If the dataset is small better to train them 1-3 epochs. For big one 10 epoch works fine. It has default early stopping of 4 epoch,
...
def fit_clf(X, y, dataset, seed):
    ...
    model.fit(X, y)
    model.save_model(f"checkpoints/{dataset}_{seed}")
    return model
...
if __name__ == '__main__':
    ...
    for seed in [1, 2, 3, 4, 5]:
        for a in clf_datasets:
            ...
            model_dir = os.path.abspath(f"checkpoints/{a}_2")
            ...
            test.to_csv(os.path.join(DATAPATH, "tdc_preds_tunetable_mordred_100_tunetables", "{}_test_{}.csv".format(a,seed)), index=False)    

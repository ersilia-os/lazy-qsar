...
def _print_summary(
    ...
    for pfx, _ in extra_rows:
        for label in ("active", "inactive"):
            ...
            mn_s  = f"{mn:.4f}"  if mn  is not None and not (isinstance(mn,  float) and np.isnan(mn))  else "—"
            p50_s = f"{p50:.4f}" if p50 is not None and not (isinstance(p50, float) and np.isnan(p50)) else "—"
            mx_s  = f"{mx:.4f}"  if mx  is not None and not (isinstance(mx,  float) and np.isnan(mx))  else "—"
...
def train_dataset(pathogen: str, dataset: str, mode: str, repo_dir: str, output_dir: str, models_output_dir: str) -> None:
    metadata_path = os.path.join(repo_dir, "data", "processed", pathogen, "01_chembl_datasets.csv")
    ...
    extra_stats: dict = {}
    for method, prefix in [("predict_score", "score"), ("predict_rank", "rank"), ("predict_lift", "lift")]:
    ...
    _print_summary(pathogen, dataset, mode, lazy_auc, lazy_aupr, lazy_bedroc, aupr_baseline,
                   ...
                   extra_stats, ef_stats)
    _save_run_csv(output_dir, pathogen, dataset, mode, len(smiles_train), len(smiles_test),
                  ...
                  calibration_y, extra_stats, ef_stats)
...
def main(args: argparse.Namespace) -> None:
    if args.csv:
        _run_batch(args.csv, args.modes, args.repo_dir, args.output_dir, args.models_output_dir, skip_existing=args.skip_existing)
    else:
        train_dataset(args.pathogen, args.dataset, args.mode, args.repo_dir, args.output_dir, args.models_output_dir)


def _run_batch(csv_path: str, modes: list[str], repo_dir: str, output_dir: str, models_output_dir: str, skip_existing: bool = False) -> None:

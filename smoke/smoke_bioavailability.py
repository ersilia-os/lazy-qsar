#!/usr/bin/env python
"""
Smoke test on the bioavailability_ma dataset (pre-computed Morgan fingerprints).
...
def _load(split):
    with h5py.File(os.path.join(DATA_DIR, f"bioavailability_ma_{split}.h5"), "r") as f:
        X = f["Values"][:]
    with open(os.path.join(DATA_DIR, f"bioavailability_ma_{split}.csv")) as f:
        reader = csv.reader(f)
        next(reader)
...
ax_cal.set_title("Calibration — Bioavailability (Ma) test set")
...
plt.tight_layout()
out_path = os.path.join(os.path.dirname(__file__), "calibration_bioavailability.png")
...
fig.suptitle(
    f"Bioavailability (Ma) — n_test={len(y_test)}  pos_rate={y_test.mean():.1%}  AUC={lazy_auc:.3f}",
    fontsize=11,
)
fig.tight_layout()
swarm_path = os.path.join(os.path.dirname(__file__), "swarm_bioavailability.png")

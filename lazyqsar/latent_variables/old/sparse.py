...
class KFeaturesReducer(object):
    ...
    def fit(self, X, y):
        ...
        self.tfidf_.fit(X_nonconst, y)
        X_tfidf = self.tfidf_.transform(X_nonconst)
    ...
    def transform(self, X):
        check_is_fitted(self, ["var_filter_", "tfidf_", "selector_"])
        X_nonconst = self.var_filter_.transform(X)
        X_tfidf = self.tfidf_.transform(X_nonconst)
        X_selected = self.selector_.transform(X_tfidf)
        return X_selected


def get_k_features_parameter(X, y):

    def sparse_k_features_initial_limits(X, y):
        ...
        tfidf_.fit(X, y)
        X = tfidf_.transform(X)
        selector_ = SelectKBest(score_func=chi2, k="all")
        selector_.fit(X, y)
    ...
    min_k_features, max_k_features, seed_k_features = sparse_k_features_initial_limits(X, y)
...
def get_latent_space_parameters(X, y, k_features):
    ...
    for train_idx, test_idx in splitter.split(X, y):
        ...
        y_tr, y_te = y[train_idx], y[test_idx]
        reducer = TruncatedSVD(max_n_components, random_state=42, algorithm="randomized")
...
class SparseDimReducerBinaryClassification(object):
    def __init__(self, 
                 n_components, 
                 k_features,
                 random_state=None):
    ...
    def fit(self, X, y):
        ...
        self.tfidf_ = TfidfTransformer()
        X_tfidf = self.tfidf_.fit_transform(X_nonconst, y)
        k = min(self.k_features, X.shape[1])
        self.selector_ = SelectKBest(score_func=chi2, k=k)
        X_selected = self.selector_.fit_transform(X_tfidf, y)
        ...
        self.reducer_.fit(X_selected, y)
        return self

    def transform(self, X, y=None):
        check_is_fitted(self, ["var_filter_", "tfidf_", "selector_", "reducer_"])
        X_nonconst = self.var_filter_.transform(X)
        X_tfidf = self.tfidf_.transform(X_nonconst)
        X_selected = self.selector_.transform(X_tfidf)

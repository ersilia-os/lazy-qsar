...
class DescriptorPortfolio:
    ...
    def select(self, smiles_list: list, y=None) -> list:
        ...
        if not applicable:
            ...
            applicable = [(name, get_descriptor_type(name)()) for name in self._all_names]
        ...
        if y is None or len(applicable) <= 1:
            ...
            screen_rows = [
                {"name": name, "n_features": None, "proxy_auc": None, "status": "skipped"}
    ...
    def _proxy_screen(self, smiles_list: list, y: np.ndarray, applicable: list) -> list:
        ...
        if ratio > _IMBALANCE_THRESHOLD:
            ...
            logger.info(f"  Using full dataset ({len(batch_idx):,} samples) for proxy CV")
        ...
        for name, desc in applicable:
            logger.info(f"  [{name}] Computing features for {len(smiles_list):,} SMILES...")
        ...
        logger.info(
            f"  Reference descriptor: '{ref_name}'  solo AUC = {ref_auc:.4f}"
        )
        ...
        logger.info(f"  Ensemble initialized with '{ref_name}'  AUC = {ensemble_auc:.4f}")
        ...
        for name, desc in candidates:
            if len(passing) >= _MAX_DESCRIPTORS:
                logger.info(f"  [{name}] SKIPPED — ensemble cap of {_MAX_DESCRIPTORS} reached")
                continue
        ...
        logger.rule()
        return [(name, desc, feature_map[name], solo_aucs[name]) for name, desc in passing]
